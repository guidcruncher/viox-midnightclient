<template>
  ...
  >
    <div class="bg-white/5 rounded-2xl p-5 flex flex-col gap-4" v-if="loaded">
...
const updateConfig = async () => {
  if (config.value) {
    await ApiClient.updateConfig(config.value)
  }
}

onMounted(async () => {
  loaded.value = false
  config.value = (await ApiClient.getConfig()).data
  loaded.value = true
})
</script>
dq
