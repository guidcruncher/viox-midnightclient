...
const toMediaType = (item: UnifiedSearchResult) => {
  return {
    id: item.id,
    title: item.title,
    subtitle: "",
...
<template>
  <button
    @click="$emit('play', toMediaType(item))"
  ...
  >
    <button
      @click.stop="$emit('toggle-favourite', toMediaType(item))"
    ...
    <div class="absolute bottom-0 left-0 w-full p-4">
      <div class="mb-1 flex items-center gap-2">
        <span class="text-[10px] font-bold uppercase tracking-wider text-white/60">
          {{ item.type || item.format || item.backend || "Media" }}
          {{ getBackEnd(item.id) }}
        </span>
      </div>
      ...
      <p class="truncate text-sm text-white/70">{{ item.artist }}</p>
    </div>
