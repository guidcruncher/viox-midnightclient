...
const emit = defineEmits(["load-more", "playall", "favourite", "click", "subscribe"]);
...
onMounted(async () => {
  ...
  if (props.parent) {
    if (
      props.parent.sourceRef.itemType === "podcast" ||
      props.parent.sourceRef.itemType === "show"
    ) {
        ...
        <div class="flex-1 space-y-6 text-center md:text-left">
          ...
          >
            ...
            <button
              v-if="canSubscribe"
              class="p-4 glass rounded-xl hover:bg-white/10 transition-all text-white border-white/10"
              @click="$emit('subscribe', props.parent)"
            >
              <LucideIcon v-if="!props.parent.subscribed" name="Eye" size="20" />
              <LucideIcon v-else name="EyeOff" size="20" />
            </button>
          </div>
        </div>
      </section>
